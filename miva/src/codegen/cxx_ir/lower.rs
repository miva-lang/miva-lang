...
pub(crate) fn lower_expr(ctx: &mut IrContext, expr: &Expr) -> IrExpr {
    match expr {
        ...
        Expr::EIf {
            ...
            let has_panic = else_.as_ref().map_or(false, |e| is_panic(e)) || is_panic(then);
            IrExpr::IfValue {
        ...
        Expr::ECast { expr, to, .. } => IrExpr::Cast {
            expr: Box::new(lower_expr(ctx, expr)),
            to: to.clone(),
        },
        Expr::EBlock { stmts, result, .. } => {
            if result.is_some() {
                let lowered_stmts: Vec<IrStmt> =
                    stmts.iter().flat_map(|s| lower_stmt(ctx, s)).collect();
                IrExpr::Block {
                    stmts: lowered_stmts,
                    result: result.as_ref().map(|e| Box::new(lower_expr(ctx, e))),
                }
            } else if stmts
                .last()
                .map_or(false, |s| matches!(s, Stmt::SExpr { .. }))
            {
                let (non_last, last) = stmts.split_at(stmts.len() - 1);
                let lowered_stmts: Vec<IrStmt> =
                    non_last.iter().flat_map(|s| lower_stmt(ctx, s)).collect();
                if let Stmt::SExpr { expr, .. } = &last[0] {
                    IrExpr::Block {
                        stmts: lowered_stmts,
                        result: Some(Box::new(lower_expr(ctx, expr))),
                    }
                } else {
                    unreachable!()
                }
        ...
        Expr::EAddr { expr, .. } => IrExpr::Addr(Box::new(lower_expr(ctx, expr))),
        Expr::EDeref { expr, .. } => IrExpr::Deref(Box::new(lower_expr(ctx, expr))),
        ...
        Expr::EChoose {
            ...
            let lowered_cases: Vec<IrCase> = cases
                .iter()
                .map(|c| {
...
pub(crate) fn lower_def(ctx: &mut IrContext, def: &Def) -> IrDef {
    match def {
        ...
        Def::SImportAs { path, alias, .. } => IrDef::ImportAs {
            path: path.clone(),
            alias: alias.clone(),
        },
        Def::SImportHere { path, .. } => IrDef::ImportHere { path: path.clone() },
        Def::DCMagical { content, .. } => IrDef::CMagical {
            content: content.clone(),
        },
        Def::DCIntro { content, .. } => IrDef::CIntro {
            content: content.clone(),
        },
    }
}
