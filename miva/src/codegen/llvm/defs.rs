...
pub(crate) fn gen_stmt(stmt: &Stmt, ctx: &mut LlvmCtx, body: &mut String) {
    match stmt {
        Stmt::SLetTuple { patterns, expr, .. } => {
            ...
            let elem_types = infer_expr_type(expr, ctx).and_then(|t| match t {
                Typ::TTuple { elems } => Some(elems),
                _ => None,
            });
            for (i, name) in patterns.iter().enumerate() {
                ...
                if let Some(Some(et)) = elem_types.as_ref().map(|elems| elems.get(i)) {
                    if typ_is_string(et) {
                        ctx.string_regs.insert(reload);
                    }
                    ctx.var_types.insert(name.clone(), et.clone());
                }
            }
        }
        Stmt::SLet {
            ...
            // Record the binding's type so later field access / printing can
            // tell tuples (offset 0), enum payloads (offset 1), and string or
            // bool members apart. Covers lambdas, calls, tuple/struct literals.
            if let Some(typ) = infer_expr_type(expr, ctx) {
                ctx.var_types.insert(name.clone(), typ);
            }
        ...
        Stmt::SExpr { expr, .. } => {
            gen_expr(expr, ctx, body);
        }
        Stmt::SCIntro { content, .. } => {
            body.push_str(&format!("  ; {}\n", content));
        }
        Stmt::SEmpty { .. } => {}
    }
...
pub(crate) fn gen_func_def(
    ...
    let mut ctx = LlvmCtx::with_module_and_fields(
        module,
...
pub(crate) fn gen_closure_thunk(
    ...
    let mut body_str = String::new();
    let ret_val = if let Expr::EBlock {
        stmts,
        result: None,
        ..
    } = body
...
pub(crate) fn gen_cfunc(name: &str, params: &[Param], returns: &Option<Typ>, code: &str) -> String {
    ...
    let param_strs: Vec<String> = params
        .iter()
        .map(|p| match p {
            Param::PRef { typ, .. } | Param::POwn { typ, .. } => match typ {
                Typ::TFloat64 => "double".to_string(),
                Typ::TFloat32 => "float".to_string(),
                Typ::TBool | Typ::TChar => "i8".to_string(),
                _ => "i64".to_string(),
            },
        })
        .collect();
    ...
    out.push_str("entry:\n");
    out.push_str(&format!(
        "  %args = alloca [{} x %MivaValue], align 8\n",
        params.len()
    ));
    out.push_str(&format!(
        "  %args_ptr = getelementptr [{} x %MivaValue], ptr %args, i64 0, i64 0\n",
        params.len()
    ));
    for (i, p) in params.iter().enumerate() {
            ...
            Param::PRef { typ, .. } | Param::POwn { typ, .. } => {
                match typ {
                    ...
                    Typ::TFloat64 | Typ::TFloat32 => {
                        ...
                        out.push_str(&format!(
                            "  %arg{}_bits = bitcast double {} to i64\n",
                            i, arg_reg
                        ));
                        out.push_str(&format!("  %arg{}_data = getelementptr %MivaValue, ptr %args_ptr, i64 {}, i32 1\n", i, i));
                    ...
                    _ => {
                        ...
                        out.push_str(&format!(
                            "  %arg{}_cstr_ptr = inttoptr i64 {} to ptr\n",
                            i, arg_reg
                        ));
                        out.push_str(&format!(
                            "  %arg{}_cstr = call ptr @miva_string_c_str(ptr %arg{}_cstr_ptr)\n",
                            i, i
                        ));
                        out.push_str(&format!("  %arg{}_data = getelementptr %MivaValue, ptr %args_ptr, i64 {}, i32 1\n", i, i));
    ...
    out.push_str(&format!(
        "  %result = call %MivaValue @miva_host_{}(ptr %args_ptr, i32 {})\n",
        name,
        params.len()
    ));
...
pub(crate) fn llvm_enum_def(name: &str, variants: &[crate::ast::EnumVariant]) -> String {
    ...
    for (idx, v) in variants.iter().enumerate() {
        ...
        out.push_str("entry:\n");
        out.push_str(&format!(
            "  %p = call ptr @miva_alloc(i64 {})\n",
            (max_fields + 1) * 8
        ));
        out.push_str(&format!(
            "  %tg = getelementptr i64, ptr %p, i64 0\n  store i64 {}, ptr %tg\n",
            idx
        ));
        for i in 0..nargs {
            out.push_str(&format!(
...
pub(crate) fn gen_impl(_struct_name: &str, impls: &[ImplExpr]) -> String {
    ...
    for impl_expr in impls {
        out.push_str(&format!(
            "; impl operator for {:?} (user function {})\n",
...
pub(crate) fn generate_with_scope(
    defs: &[Def],
    module: Option<&str>,
    struct_field_map: &HashMap<String, HashMap<String, usize>>,
    struct_field_types: &HashMap<String, HashMap<String, Typ>>,
    func_sigs: &HashMap<String, crate::codegen::FuncSig>,
) -> (String, String, String, HashSet<String>) {
    ...
    for def in defs {
        match def {
            Def::DFunc {
                ...
                body,
                ..
            } if name == "main" => main_functions.push_str(&gen_main_func(
                body,
                struct_field_map,
                struct_field_types,
                func_sigs,
            ...
            Def::DFunc {
                ...
                let global_name = make_global_name(module, name.as_str());
                defined.insert(global_name);
                defs_str.push_str(&gen_func_def(
                    name,
                    type_params,
                    params,
                    returns,
            ...
            Def::DImpl {
                struct_name, impls, ..
            } => defs_str.push_str(&gen_impl(struct_name, impls)),
            Def::DModule { name, .. } => {
                let inner = generate_with_scope(
                    &defs[1..],
                    Some(name.as_str()),
                    struct_field_map,
                    struct_field_types,
                ...
                struct_defs.push_str(&inner.0);
                defs_str.push_str(&inner.1);
                main_functions.push_str(&inner.2);
...
pub(crate) fn generate_bridge(_defs: &[Def]) -> String {
    ...
    bridge.push_str(
        "void miva_println(void* s) { auto& str = *(std::string*)s; mvp_println(str); }\n",
    );
    bridge
        .push_str("void miva_prints(void* s) { auto& str = *(std::string*)s; mvp_prints(str); }\n");
    bridge.push_str(
        "void miva_printlns(void* s) { auto& str = *(std::string*)s; mvp_printlns(str); }\n",
    );
    bridge.push_str("void miva_error(void* s) { auto& str = *(std::string*)s; mvp_error(str); }\n");
    bridge
        .push_str("void miva_errors(void* s) { auto& str = *(std::string*)s; mvp_errors(str); }\n");
    bridge.push_str(
        "void miva_errorln(void* s) { auto& str = *(std::string*)s; mvp_errorln(str); }\n",
    );
    bridge.push_str(
        "void miva_errorlns(void* s) { auto& str = *(std::string*)s; mvp_errorlns(str); }\n",
    );
    ...
    bridge.push_str("int64_t miva_string_eq(void* a, void* b) { return (*(std::string*)a) == (*(std::string*)b) ? 1 : 0; }\n");
    bridge.push_str(
        "int64_t miva_string_parse(void* s) { return mvp_string_parse(*(std::string*)s); }\n",
    );
    bridge.push_str(
        "int64_t miva_string_length(void* s) { return mvp_string_length(*(std::string*)s); }\n",
    );
    bridge.push_str("void* miva_string_make(void* init, int64_t size) { return new std::string(mvp_string_make(*(std::string*)init, size)); }\n");
    bridge.push_str(
        "void* miva_string_from_int(int64_t v) { return new std::string(mvp_to_string(v)); }\n",
    );
    bridge.push_str(
        "void* miva_string_from_float(double v) { return new std::string(mvp_to_string(v)); }\n",
    );
    bridge.push_str(
        "void* miva_string_from_bool(int8_t v) { return new std::string(mvp_to_string(v)); }\n",
    );
    bridge.push_str("void* miva_string_from_str(const char* s) { return new std::string(s); }\n");
    bridge.push_str(
        "void* miva_string_c_str(void* s) { return (void*)strdup(((std::string*)s)->c_str()); }\n",
    );
    ...
    bridge.push_str(
        "int64_t miva_box_deref_int(void* b) { return **(mvp_builtin_box<mvp_builtin_int>*)b; }\n",
    );
    ...
    bridge.push_str(
        "int8_t miva_box_deref_byte(void* b) { return **(mvp_builtin_box<mvp_builtin_byte>*)b; }\n",
    );
    ...
    bridge.push_str(
        "int64_t miva_json_kind(int64_t v) { return mvp_json_kind((void*)(intptr_t)v); }\n",
    );
    bridge.push_str(
        "int64_t miva_json_bool(int64_t v) { return mvp_json_bool((void*)(intptr_t)v); }\n",
    );
    ...
    bridge.push_str(
        "int64_t miva_xml_kind(int64_t v) { return mvp_xml_kind((void*)(intptr_t)v); }\n",
    );
    ...
    bridge.push_str(
        "int64_t miva_toml_kind(int64_t v) { return mvp_toml_kind((void*)(intptr_t)v); }\n",
    );
    bridge.push_str(
        "int64_t miva_toml_bool(int64_t v) { return mvp_toml_bool((void*)(intptr_t)v); }\n",
    );
    ...
    bridge.push_str(
        "int64_t miva_yaml_kind(int64_t v) { return mvp_yaml_kind((void*)(intptr_t)v); }\n",
    );
    bridge.push_str(
        "int64_t miva_yaml_bool(int64_t v) { return mvp_yaml_bool((void*)(intptr_t)v); }\n",
    );
    ...
    bridge
        .push_str("void miva_mutex_unlock(int64_t h) { mvp_mutex_unlock((void*)(intptr_t)h); }\n");
